...
public class BasicDataFormatter implements Formatter<Object> {
  ...
  private static final DateTimeFormat longDateFormat = DateTimeFormat
      .getFormat("dd MMM, yyyy hh:mm Z");
  ...
  public interface CouponTemplate extends SafeHtmlTemplates {
    @Template("<div class='couponDiv'><h4>{0}</h4>" + "<ul>"
    ...
    SafeHtml getCouponDetailsTemplate(String title,
        String description,
        BigDecimal price,
        String discount,
        Long quantity,
    ...
    @Template("<div class='couponDiv'><h4>{0}</h4>"
        + "<div class='coupon_image'><image src='{7}' /></div>" + "<ul>"
        + "<li><span class='heading'>Description</span> {1}$</li>"
        + "<li><span class='heading'>Regular Price</span> {2}$</li>"
        + "<li><span class='heading'>Discount</span> {3}</li>"
        + "<li><span class='heading'>Quantity remaining</span> {4}</li>"
  ...
  public String format(Object value, ValueType type) {
    switch (type) {
      ...
      case DATE_VALUE_MONTH_DAY:
        String date = shortDateFormat.format((Date) value, TimeZone.createTimeZone(timeZoneConstants.americaLosAngeles()));
        return date;
      case DATE_VALUE_FULL:
        date = longDateFormat.format((Date) value, TimeZone.createTimeZone(timeZoneConstants.americaLosAngeles()));
        return date;
  ...
  public static String getContent(String content) {
    ...
    while (foundPattern) {
      ...
      if (matcher != null) {
        ...
        int patternEndIndex = patternStartIndex + group.length();// content.indexOf(' ',
                                                                 // patternStartIndex);
...
//      temp -= days * (60 * 60 * 24);
//      String daysKey = days > 1 ? "days" : "day";
//      sb.append(timeTemplate.timeElapsed(days, daysKey).asString());
//    }
//
//    if (temp > 60 * 60) {
...
//      temp -= hours * (60 * 60);
//      String hoursKey = hours > 1 ? "hours" : "hour";
//      sb.append(" " + timeTemplate.timeElapsed(hours, hoursKey).asString());
//    }
//
//    if (temp > 60) {
//      long mins = temp / 60;
//      temp -= mins * 60;
//      String minsKey = mins > 1 ? "mins" : "min";
//      sb.append(" " + timeTemplate.timeElapsed(mins, minsKey).asString());
//    }
//
//    if (temp > 0) {
//      String secondsKey = seconds > 1 ? "seconds" : "second";
//      sb.append(" " + timeTemplate.timeElapsed(temp, secondsKey).asString());
//    }
//
//    return sb.toString();
//  }
  ...
  public static void main(String[] args) {
    ...
    String content2 =
        "Take a look at this - http://www.yahoo.com/susnata/01923.html, http://www.ziplly.com";

...
//	ITweetView<TweetPresenter> tview = new TweetView();
	...
	public HomeViewImpl(
		...
		tweetBox = new TweetBox(eventBus, tweetBoxPresenter);
	...
	@Override
	public void insertLast(List<TweetDTO> tweets, RenderingStatusCallback callback) {
	...
	public void display(List<TweetDTO> tweets) {
		...
		tview.insertLast(tweets, new RenderingStatusCallback() {
  ...
  public TweetListView getTweetView() {
	...
	@Override
	public void insertFirst(TweetDTO tweet) {
		tview.insertFirst(tweet);
	}
		...
		for (final TweetType type : TweetType.values()) {
			...
			anchor.addClickHandler(new ClickHandler() {
				...
				public void onClick(ClickEvent event) {
					...
					presenter.displayCommunityWall(type);
				}
			});

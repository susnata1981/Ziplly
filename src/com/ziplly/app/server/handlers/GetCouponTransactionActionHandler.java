...
public class GetCouponTransactionActionHandler extends
    AbstractAccountActionHandler<GetCouponTransactionAction, GetCouponTransactionResult> {
  ...
  public GetCouponTransactionResult doExecute(GetCouponTransactionAction action,
    ...
    result.setPurchasedCoupons(EntityUtil.cloneCouponItems(details.getPurchasedCoupons()));
  ...
  private TransactionDetails searchByCouponId(GetCouponTransactionAction action) {
    ...
    details.setPurchasedCoupons(orderDao.findTransactionByCouponId(
        action.getCouponId(),
        action.getStart(),
        action.getPageSize()));
  ...
  private TransactionDetails searchByAccountId(GetCouponTransactionAction action) {
    TransactionDetails details = new TransactionDetails();
    details.setPurchasedCoupons(orderDao.findByAccountIdAndStatus(session
        .getAccount()
        .getAccountId(), TransactionStatus.COMPLETED, action.getStart(), action.getPageSize()));

    details.setTotalTransactionCount(orderDao.getTotalCountByAccountIdAndStatus(session
        .getAccount()
        .getAccountId(), TransactionStatus.COMPLETED));
    return details;
  }

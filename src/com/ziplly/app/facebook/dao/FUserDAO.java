...
//		getUserFriendDetails(user.getId());
	...
	public LatLong getLocationInfo(User user) {
		...
		logger.log(Level.INFO, String.format("LocationData(%s): %s",user.getLocation().getId(),locationData));
	...
	public FacebookUserStats getUserFriendDetails(String uid) {
		...
		query = String.format("select uid, first_name, current_location from user where uid in %s",param);
...
//				post.description = p.getDescription();
	...
	public MQueryResults getUserStatusDetails(String uid) {
		Map<String, String> queries = ImmutableMap.of(
				"statusList", String.format("Select like_info.like_count,time from status where uid = %s", uid));
		...
		System.out.println("Result size="+mqr.statusList.size()+" count(0)="+mqr.statusList.get(0).likeInfo.likeCount
				+" time="+time);
		return mqr;
	}
	...
	public FacebookUserStats getUserLikeDetails(String uid) {
		FacebookUserStats fus = new FacebookUserStats();
		Map<String, String> queries = ImmutableMap.of(
				"likes", String.format("Select object_type from like where user_id = %s", uid));
		MQueryResults mqr = fbClient.executeFqlMultiquery(queries, MQueryResults.class);

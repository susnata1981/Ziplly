...
public class NeighborhoodDAOImpl extends BaseDAO implements NeighborhoodDAO {
  ...
  public List<Neighborhood>
      findDescendentNeighborhoods(Long neighborhoodId) throws NotFoundException {
    Query query =
        getEntityManager().createQuery(
  ...
  /**
   * Given a neighborhood, find out all the neighborhood id's of that city.
   */
  @Override
  public List<Long> getNeigborhoodIdsForCity(Long neighborhoodId) {
    try {
      NeighborhoodDTO neighborhood = findById(neighborhoodId);
      List<Neighborhood> allNeighborhoods = findDescendentNeighborhoods(
          neighborhood.getParentNeighborhood().getNeighborhoodId());
      
      List<Long> allNeighborhoodIds = Lists.newArrayList(convertToNeighborhoodIds(allNeighborhoods));
      allNeighborhoodIds.add(neighborhood.getParentNeighborhood().getNeighborhoodId());
  ...
  private List<Long> convertToNeighborhoodIds(List<Neighborhood> allNeighborhoods) {
    return Lists.transform(allNeighborhoods, new Function<Neighborhood, Long>() {
